...
class hn_captcha
{
	...
	//
		...
		function hn_captcha($config,$secure=TRUE)
		{
			...
			if(
				...
				||
				(isset($_POST['captcharefresh']) && isset($_POST['private_key']) && $priv)
				)
			{
	...
	//
		...
		/**
		  ...
		  *
		  **/
		function display_form($ts, $new, $short_http_path)
		{
...
if ($new)
			$s .= '<p class="captcha_notvalid"></p>';
else
			$s .= '<p class="captcha_notvalid">'.$this->notvalid_msg().'</p>';

                        $s .= '<table>';
                        $s .= '<tr>';
                        $s .= '<td rowspan=2 VALIGN="bottom">';
			$s .= '<p class="captcha_1">'.$this->display_captcha($ts, FALSE, $short_http_path)."</p>\n";
                        $s .= '</td>';
                        $s .= '<td VALIGN="top">';
		...
		function display_form_2($ts, $short_http_path)
		{
                        $s .= '</td></tr><tr><td VALIGN="bottom"><p class="captcha_1"><input class="captcha" type="text" name="private_key" size=7 value="" maxlength="'.$this->chars.'" size="'.$this->chars.'">&nbsp;&nbsp;';
                        $s .= '</p></tr></td>';
                        $s .= '<tr></table>';
			$s .= '<p>'.$this->msg1.'<br>';
                        $edata = base64_encode($ts);
                        $s .= '&nbsp;&nbsp;&nbsp;Can\'t see or understand the image?';
                        $s .= '<ul>';
                        $s .= '	<li>Click on the image to fetch a new one.</li>';
                        $s .= '	<li>Click on the Play button above to listen hear what is required to be typed. (Flash Required)</li>';
                        $s .= '	<li><a href="/lib/plugins/exempt/audio-check.php?data='.$edata.'&file=mp3">Click here for an audible mp3 version</a>.</li>';
                        $s .= '	<li><a href="/lib/plugins/exempt/audio-check.php?data='.$edata.'&file=wav">Click here for an audible wav version</a>.</li>';
			$s .= '</ul></p>';
			$s .= '<p>&nbsp;&nbsp;&nbsp;<button name="submit" type="submit">Continue</button></p>';
			$s .= '</div>';
			return $s;
		}
	...
	//
		...
		/** @private **/
		function display_captcha($ts, $onlyTheImage=FALSE, $short_http_path)
		{
			$this->make_captcha($ts);
			$is = getimagesize($this->get_filename());
			if($onlyTheImage) return "\n".'<img onClick="location.reload()" style="cursor: pointer; cursor: hand;" class="captchapict" src="'.$this->get_filename_url("", $short_http_path).'" '.$is[3].' alt="This is a captcha-picture. It is used to prevent mass-access by robots. (see: www.captcha.net)" title="">'."\n";
...
//			include("/data/web/shared/dokuwiki/lib/plugins/exempt/sqlite.php");
		...
		/** @private **/
		function get_filename_url($public="", $short_http_path)
		{
			if($public=="") $public = $this->public_key;
return "lib/tmp/".$public.".jpg";
